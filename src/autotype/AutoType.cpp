...
AutoType::parseSequence(const QString& entrySequence, const Entry* entry, QString& error, bool syntaxOnly)
{
    ...
    int cumulativeDelay = config()->get(Config::AutoTypeStartDelay).toInt();
    int currentTypingDelay = config()->get(Config::AutoTypeDelay).toInt();
    ...
    actions << QSharedPointer<AutoTypeDelay>::create(qBound(0, currentTypingDelay, maxTypeDelay), true);
    actions << QSharedPointer<AutoTypeDelay>::create(qBound(0, cumulativeDelay, maxWaitDelay));
